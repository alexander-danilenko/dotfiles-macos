...
const colorDanger = '\x1b[38;2;220;20;60m'; // Fable / Mythos — crimson #DC143C (cost warning)
...
const shortenedRaw = home && cwd.startsWith(home) ? '~' + cwd.slice(home.length) : cwd;
...
if (branch) {
  ...
  const label = branch === 'HEAD' ? git(cwd, ['rev-parse', '--short', 'HEAD']) : branch;
...
if (ctxWindow != null) {
  ...
  ctxSizeLabel = w >= 1000000 ? `${Math.round(w / 1000000)}M` : `${Math.round(w / 1000)}K`;
}
...
if (modelId) {
  ...
  let family;
  let modelColor;
  if (modelLower.includes('fable')) {
    family = 'fable';
    modelColor = colorDanger;
  } else if (modelLower.includes('mythos')) {
    family = 'mythos';
    modelColor = colorDanger;
  ...
  const stripped = modelLower.replace(/claude-/g, '').split(family).join('').replace(/^-/, '').replace(/-$/, '');
